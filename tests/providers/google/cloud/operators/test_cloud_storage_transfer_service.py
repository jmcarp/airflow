...
class TestTransferJobValidator(unittest.TestCase):
    def test_should_raise_exception_when_encounters_aws_credentials(self):
        ...
        err = cm.exception
        assert "AWS credentials detected inside the body parameter (awsAccessKey). This is not allowed, please " \
            "use Airflow connections to store credentials." in \
            str(err)
    ...
    def test_verify_data_source(self, transfer_spec):
        ...
        with pytest.raises(AirflowException) as cm:
            TransferJobValidator(body=body).validate_body()
        err = cm.exception
        assert "More than one data source detected. Please choose exactly one data source from: " \
            "gcsDataSource, awsS3DataSource and httpDataSource." in \
            str(err)
