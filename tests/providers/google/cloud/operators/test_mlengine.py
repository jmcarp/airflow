...
class TestMLEngineBatchPredictionOperator(unittest.TestCase):
    ...
    def test_invalid_model_origin(self):
        ...
        assert 'Ambiguous model origin: Both uri and ' \
        ...
        with pytest.raises(AirflowException) as context:
            MLEngineStartBatchPredictionJobOperator(**task_args).execute(None)
        assert 'Missing model: Batch prediction expects a model ' \
        ...
        with pytest.raises(AirflowException) as context:
            MLEngineStartBatchPredictionJobOperator(**task_args).execute(None)
        assert 'Missing model origin: Batch prediction expects a ' \
    ...
    def test_http_error(self, mock_hook):
        ...
        assert http_error_code == context.exception.resp.status
    ...
    def test_failed_job_error(self, mock_hook):
        ...
        assert 'A failure message' == str(context.exception)
...
class TestMLEngineTrainingOperator(unittest.TestCase):
    ...
    def test_console_extra_link(self, mock_hook):
        ...
        assert f"https://console.cloud.google.com/ai-platform/jobs/{job_id}?project={project_id}" == \
