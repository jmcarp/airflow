...
deftest_gen_conn(self,mock_build,mock_authorize):
    ...
    mock_build.assert_called_once_with(    "doubleclicksearch",    API_VERSION,    http=mock_authorize.return_value,    cache_discovery=False,    )
    assert mock_build.return_value == result

    @mock.patch("airflow.providers.google.marketing_platform.hooks.""search_ads.GoogleSearchAdsHook.get_conn")
deftest_insert(self,get_conn_mock):
    report = {"report": "test"}
    return_value = "TEST"
    get_conn_mock.return_value.reports.return_value.request.return_value.execute.return_value = (    return_value    )
    ...
    @mock.patch("airflow.providers.google.marketing_platform.hooks.""search_ads.GoogleSearchAdsHook.get_conn")
deftest_get(self,get_conn_mock):
    report_id = "REPORT_ID"
    return_value = "TEST"
    ...
    @mock.patch("airflow.providers.google.marketing_platform.hooks.""search_ads.GoogleSearchAdsHook.get_conn")
deftest_get_file(self,get_conn_mock):
    report_fragment = 42
    report_id = "REPORT_ID"
    return_value = "TEST"
    get_conn_mock.return_value.reports.return_value.getFile.return_value.execute.return_value = (    return_value    )
    result = self.hook.get_file(    report_fragment=report_fragment, report_id=report_id    )
    get_conn_mock.return_value.reports.return_value.getFile.assert_called_once_with(    reportFragment=report_fragment, reportId=report_id    )
    assert return_value == result
