...
def setUp(self):
    dag_id = 'test_dag'
    self.dag = DAG(    dag_id,    default_args={    'owner': 'airflow',    'start_date': datetime.datetime.today(),    'end_date': datetime.datetime.today() + datetime.timedelta(days=1),    },    schedule_interval='@daily'    )
...
deftest_create_cluster(self,create_cluster_mock,*_):
    operator = DataprocCreateClusterOperator(    task_id='create_cluster',    ssh_public_keys=SSH_PUBLIC_KEYS,    folder_id=FOLDER_ID,    subnet_id=SUBNET_ID,    zone=AVAILABILITY_ZONE_ID,    connection_id=CONNECTION_ID,    s3_bucket=S3_BUCKET_NAME_FOR_LOGS,    cluster_image_version=CLUSTER_IMAGE_VERSION,    )
    context = {'task_instance': MagicMock()}
    operator.execute(context)
    create_cluster_mock.assert_called_once_with(    cluster_description='',    cluster_image_version='1.1',    cluster_name=None,    computenode_count=0,    computenode_disk_size=15,    computenode_disk_type='network-ssd',    computenode_resource_preset='s2.small',    datanode_count=2,    datanode_disk_size=15,    datanode_disk_type='network-ssd',    datanode_resource_preset='s2.small',    folder_id='my_folder_id',    masternode_disk_size=15,    masternode_disk_type='network-ssd',    masternode_resource_preset='s2.small',    s3_bucket='my_bucket_name',    service_account_id=None,    services=('HDFS', 'YARN', 'MAPREDUCE', 'HIVE', 'SPARK'),    ssh_public_keys=[    'ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAYQCxO38tKAJXIs9ivPxt7AYdfybgtAR1ow3Qkb9GPQ6wkFHQqcFD'    'e6faKCxH6iDRteo4D8L8BxwzN42uZSB0nfmjkIxFTcEU3mFSXEbWByg78aoddMrAAjatyrhH1pON6P0='    ],    subnet_id='my_subnet_id',    zone='ru-central1-c'    )
    context['task_instance'].xcom_push.assert_has_calls([    call(key='cluster_id', value=create_cluster_mock().response.id),    call(key='yandexcloud_connection_id', value=CONNECTION_ID),    ])

    @patch('airflow.providers.yandex.hooks.YandexCloudBaseHook._get_credentials')
@patch('airflow.hooks.base_hook.BaseHook.get_connection')
@patch('yandexcloud._wrappers.dataproc.Dataproc.delete_cluster')
deftest_delete_cluster_operator(self,delete_cluster_mock,*_):
    operator = DataprocDeleteClusterOperator(    task_id='delete_cluster',    connection_id=CONNECTION_ID,    )
    context = {'task_instance': MagicMock()}
