...
class TestSqoopHook(unittest.TestCase):
    ...
    def test_cmd_mask_password(self):
        ...
        """
        hook = SqoopHook()
        assert hook.cmd_mask_password(['--password', 'supersecret']) == \
            ['--password', 'MASKED']

        cmd = ['--target', 'targettable']
        assert hook.cmd_mask_password(cmd) == \
            cmd
