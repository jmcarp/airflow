...
class TestAwsBatchClient(unittest.TestCase):
    ...
    def test_init(self):
        ...
        assert self.batch_client.aws_conn_id == None
        assert self.batch_client.hook == self.aws_hook_mock.return_value
    ...
    def test_check_job_success_raises_unknown_status(self):
        ...
        assert msg in str(e.exception)
        assert status in str(e.exception)
...
class TestAwsBatchClientDelays(unittest.TestCase):
    ...
    def test_delay_defaults(self, mock_sleep, mock_uniform):
        assert AwsBatchClient.DEFAULT_DELAY_MIN == 1
        assert AwsBatchClient.DEFAULT_DELAY_MAX == 10
