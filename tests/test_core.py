...
class TestCore(unittest.TestCase):
    ...
    def test_illegal_args(self):
        ...
        with conf_vars({('operators', 'allow_illegal_arguments'): 'True'}):
            with pytest.warns(PendingDeprecationWarning) as warning:
                BashOperator(
    ...
    def test_complex_template(self):
        def verify_templated_field(context):
            assert context['ti'].task.some_templated_field['bar'][1] == \
                             context['ds']
    ...
    def test_round_time(self):
        ...
        assert datetime(2015, 1, 1, 0, 0) == rt1

        rt2 = round_time(datetime(2015, 1, 2), relativedelta(months=1))
        assert datetime(2015, 1, 1, 0, 0) == rt2

        rt3 = round_time(datetime(2015, 9, 16, 0, 0), timedelta(1), datetime(
            2015, 9, 14, 0, 0))
        assert datetime(2015, 9, 16, 0, 0) == rt3

        rt4 = round_time(datetime(2015, 9, 15, 0, 0), timedelta(1), datetime(
            2015, 9, 14, 0, 0))
        assert datetime(2015, 9, 15, 0, 0) == rt4

        rt5 = round_time(datetime(2015, 9, 14, 0, 0), timedelta(1), datetime(
            2015, 9, 14, 0, 0))
        assert datetime(2015, 9, 14, 0, 0) == rt5

        rt6 = round_time(datetime(2015, 9, 13, 0, 0), timedelta(1), datetime(
            2015, 9, 14, 0, 0))
        assert datetime(2015, 9, 14, 0, 0) == rt6

    def test_infer_time_unit(self):

        assert 'minutes' == infer_time_unit([130, 5400, 10])

        assert 'seconds' == infer_time_unit([110, 50, 10, 100])

        assert 'hours' == infer_time_unit([100000, 50000, 10000, 20000])

        assert 'days' == infer_time_unit([200000, 100000])
