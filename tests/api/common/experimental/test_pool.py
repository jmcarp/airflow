...
class TestPool(unittest.TestCase):
    ...
    def test_create_pool_bad_name(self):
        for name in ('', '    '):
            with pytest.raises(AirflowBadRequest, match="^Pool name shouldn't be empty$"):
                pool_api.create_pool(name=name,
                                   slots=5,
                                   description='')
    ...
    def test_create_pool_name_too_long(self):
        ...
        with pytest.raises(AirflowBadRequest, match="^Pool name can't be more than %d characters$" % column_length):
            pool_api.create_pool(name=long_name,
                               slots=5,
                               description='')

    def test_create_pool_bad_slots(self):
        with pytest.raises(AirflowBadRequest, match="^Bad value for `slots`: foo$"):
            pool_api.create_pool(name='foo',
                               slots='foo',
                               description='')
