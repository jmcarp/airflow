...
class TestGreaterEqualThan(unittest.TestCase):
    ...
    def test_validation_raises(self):
        ...
        with pytest.raises(validators.ValidationError, match="^Field must be greater than or equal to other field.$"):
            self._validate()

    def test_validation_raises_custom_message(self):
        self.form_field_mock.data = '2017-05-04'

        with pytest.raises(validators.ValidationError, match="^This field must be greater than or equal to MyField.$"):
            self._validate(message="This field must be greater than or equal to MyField.",)
...
class TestValidJson(unittest.TestCase):
    ...
    def test_validation_raises_custom_message(self):
        ...
        with pytest.raises(validators.ValidationError, match="Invalid JSON"):
            self._validate(message="Invalid JSON: {}",)
