...
class TestUtils(unittest.TestCase):
    ...
    def check_generate_pages_html(self, current_page, total_pages,
        ...
        assert search not in html_str, \
                         "The raw search string shouldn't appear in the output"
        assert 'search=%27%3E%22%2F%3E%3Cimg+src%3Dx+onerror%3Dalert%281%29%3E' in \
                      html_str

        assert callable(html_str.__html__), \
    ...
    def test_params_escape(self):
        assert 'search=%27%3E%22%2F%3E%3Cimg+src%3Dx+onerror%3Dalert%281%29%3E' == \
    ...
    def test_state_token(self):
        ...
        assert '&lt;script&gt;alert(1)&lt;/script&gt;' in \
            html
        assert '<script>alert(1)</script>' not in \
            html
