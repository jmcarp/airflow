...
class TestTaskInstance(unittest.TestCase):
    ...
    def test_infer_dag(self):
        ...
        # double check dags
        assert [i.has_dag() for i in [op1, op2, op3, op4]] == \
    ...
    def test_xcom_pull_after_success(self):
        ...
        assert ti.xcom_pull(task_ids='test_xcom', key=key) == None
    ...
    def test_xcom_pull_different_execution_date(self):
        ...
        assert ti.xcom_pull(task_ids='test_xcom',
                                      key=key,
    ...
    def test_previous_ti(self, _, schedule_interval, catchup) -> None:
        ...
        assert ti_list[0].previous_ti is None

        assert ti_list[2].previous_ti.execution_date == \
    ...
    def test_previous_ti_success(self, _, schedule_interval, catchup) -> None:
        ...
        assert ti_list[0].previous_ti_success is None
        assert ti_list[1].previous_ti_success is None

        assert ti_list[3].previous_ti_success.execution_date == \
            ti_list[1].execution_date
    ...
    def test_previous_execution_date_success(self, _, schedule_interval, catchup) -> None:
        ...
        assert ti_list[0].previous_execution_date_success is None
        assert ti_list[1].previous_execution_date_success is None
        assert ti_list[3].previous_execution_date_success == \
            ti_list[1].execution_date
        assert ti_list[3].previous_execution_date_success != \
            ti_list[2].execution_date
    ...
    def test_previous_start_date_success(self, _, schedule_interval, catchup) -> None:
        ...
        assert ti_list[0].previous_start_date_success is None
        assert ti_list[1].previous_start_date_success is None
        assert ti_list[3].previous_start_date_success == \
    ...
    def _env_var_check_callback(self):
        ...
        assert 'manual__' + DEFAULT_DATE.isoformat() == \
    ...
    def test_get_rendered_template_fields(self, store_serialized_dag):
        ...
        assert settings.STORE_SERIALIZED_DAGS == store_serialized_dag
