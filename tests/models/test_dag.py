...
class TestDag(unittest.TestCase):
    ...
    def test_dag_default_view_default_value(self):
        ...
        assert conf.get('webserver', 'dag_default_view').lower() == \
                         dag.default_view
    ...
    def test_dag_orientation_default_value(self):
        ...
        assert conf.get('webserver', 'dag_orientation') == \
                         dag.orientation
    ...
    def test_dag_topological_sort_dag_without_tasks(self):
        ...
        assert tuple() == dag.topological_sort()
    ...
    def test_sync_to_db(self, mock_now):
        ...
        assert orm_dag.last_scheduler_run == now
        assert orm_dag.is_active
        assert orm_dag.default_view is not None
        assert orm_dag.default_view == \
        ...
        assert orm_subdag.last_scheduler_run == now
    ...
    def test_duplicate_task_ids_not_allowed_with_dag_context_manager(self):
        ...
        assert dag1.task_dict == {op3.task_id: op3, op4.task_id: op4}
    ...
    def test_schedule_dag_no_previous_runs(self):
        ...
        assert datetime_tz(2015, 1, 2, 0, 0) == \
    ...
    def test_schedule_dag_relativedelta(self):
        ...
        assert dag.dag_id == dag_run2.dag_id
        assert dag_run2.run_id is not None
        assert '' != dag_run2.run_id
        assert datetime_tz(2015, 1, 2, 0, 0) + delta == \
            dag_run2.execution_date, \
            'dag_run2.execution_date did not match expectation: {0}' \
            .format(dag_run2.execution_date)
        assert State.RUNNING == dag_run2.state
        assert not dag_run2.external_trigger
        dag.clear()
        self._clean_up(dag_id)
    ...
    def test_fractional_seconds(self):
        ...
        assert start_date == run.execution_date, \
